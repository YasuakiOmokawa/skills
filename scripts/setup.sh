...
4 セクション（Jira / Release labels / Environments / ai-prototype-flow の DD 文書）を
順番に質問します。
...
# -----------------------------------------------------------------------------
# Section D: ai-prototype-flow の DD 文書
# -----------------------------------------------------------------------------
echo ""
echo "─── Section D: ai-prototype-flow の DD 文書 ───"
if prompt_yes_no "ai-prototype-flow を使いますか？（自組織の DD 文書を配置します）"; then
  DD_DIR="$CONFIG_DIR/ai-prototype-flow"
  mkdir -p "$DD_DIR"
  ...
  copy_dd_doc "DD テンプレート（フェーズ2.5 の DD 作成が参照）" "dd_template.md"
  copy_dd_doc "完成 DD の参考実例" "dd_reference.md"
  copy_dd_doc "差分設計 (dd_prd_delta 型) の実例（再入時に参照）" "dd_prd_delta.md"
else
  echo "  → スキップ"
fi
