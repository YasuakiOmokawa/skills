...
├── skills/<name>/references/            # 参考資料 (skill 本文から Read で参照)
├── agents/*.md                          # plugin top-level agent (auto-discovery 対象)
└── commands/<name>.md                   # slash command (create-pr plugin のみ)
```

`plugins/<name>/skills/<name>/` の 2 階層構造は Claude Code の plugin loader が `${CLAUDE_PLUGIN_ROOT}/skills/` を見るため必要。**agents/ は plugin top-level に置く** ことで Claude Code の auto-discovery 対象となり、`subagent_type="<plugin>:<agent>"` で型付け呼び出しできる。
...
| 種別 | 配置 | discovery | 呼び出し方 |
|---|---|---|---|
| Skill | `plugins/<name>/skills/<name>/SKILL.md` | 自動 (plugin.json への列挙不要) | `/<skill-name>` slash command, または skill description trigger |
| Slash command | `plugins/<name>/commands/<name>.md` | 自動 | `/<command-name>` |
| Agent (top-level) | `plugins/<name>/agents/<agent>.md` | 自動 (session start 時) | `Task(subagent_type="<plugin>:<agent>", ...)` |
| Reference doc | `plugins/<name>/skills/<name>/references/*.md` | 手動 (skill 本文/agent から Read で参照) | — |

`plugin.json` には `skills/commands/agents` 配列を**書かない**。Claude Code はファイル構造から自動 discovery する。

**重要**: agent は **plugin top-level の `agents/`** に置くこと。`plugins/<name>/skills/<name>/agents/` (skill 内サブ agent) は auto-discovery されず `subagent_type` 型付け呼び出しが機能しないため、`general-purpose` + Read で agent ファイル inline 注入というワークアラウンドが必要になる (PR #32 で全 plugin を top-level 配置に統一)。
