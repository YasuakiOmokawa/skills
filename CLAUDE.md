...
スキルを「engineering 系」「personal 系」「career 系」のバケットで**説明上**分類する。物理ディレクトリでは分けない：

- **engineering 系** — 日常の開発作業で使う汎用スキル（プロジェクト非依存）
- **personal 系** — 環境依存・組織依存のスキル（利用者が自社設定で使う前提）
- **career 系** — キャリア戦略・ビジョン整合・branding に関わるスキル。`~/.claude/skills-config/vision.md` 等のユーザー個人設定を入力に取り、記事下書き / 履歴書実績欄 / 登壇 outline 等の外部表現を生成する。プロジェクト**内部**のフェーズを扱う engineering 系とは別レイヤー（プロジェクト**全体**を入力に取りキャリアレイヤーで処理する）
...
`career 系` のスキルは **ユーザー個人設定が必須**である点が engineering 系と異なる。`~/.claude/skills-config/vision.md` などの個人設定がなければ、テンプレートをコピーして編集を促す動作とする。
