...
- **プレフィックス剥離**: 技術メモ / 備考から抽出する際、`やらない: ` `対象外: ` `AC: ` `完了条件: ` の各プレフィックスを除去し、本文のみを箇条書き化する（プレフィックス保持禁止）
- **抽出値優先**: サブタスクの完了条件は、説明・備考から抽出できる場合は **それのみ** を採用。抽出できない場合**のみ**「親US {Jiraキー} の AC に準拠」固定文言を使う。**両方併記禁止**（冗長回避）
- **「やること」分解粒度（Decision Table）**:
  ...
  - 入力が **30文字以下の短文** → 原則 1 項目（短文を機械的に分割しない）
  - 入力が **30文字超 かつ 複数動詞** → 句点 / 「+」/ 「、」で分解。1動詞 1 項目目安
