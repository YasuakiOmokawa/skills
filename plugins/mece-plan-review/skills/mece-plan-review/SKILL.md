...
- 並列起動を維持する都合上、BB が自前で wiki を読む権限は残す (順次にすると BB が WB と並列起動できなくなる)
- 重複読みは許容し、main agent が両者の結果を統合する (Wiki Researcher は事実情報、BB は判定込み、両方が成果物となる)
- ただし Wiki Researcher の責務を「BB の補助」ではなく「関連リポ横断の事実収集を一括引き受け」に格上げし、BB はカレントリポの wiki に絞る (重複範囲を最小化)
