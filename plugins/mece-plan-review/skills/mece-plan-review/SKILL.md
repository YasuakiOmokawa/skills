...
`## 受け入れ条件` を BB (仕様) / WB (コード) / Fresh Red Team の独立した視点で検証する。PoC では通常使わない。

## Inputs and tier

1. `$ARGUMENTS` のプラン、なければセッションの Plan File Info を読む。
2. `<plan>.analysis.md` の `## 受け入れ条件` がなければ、その絶対パスと `/define-acceptance-criteria` が必要な旨を報告して終了する。
3. [references/init-common.md](references/init-common.md) と [references/ac-enumerate.md](references/ac-enumerate.md) に従い、AC を `AC-N` へ正規化する。`${CODE_ROOT}` は plan の対象 repo、実装ファイルの共通 Git root、現在の Git root の順で解決する。辿れなければ `(対象コード不在)`。
...
- Red Team には plan / AC 本文を渡さず、BB / WB の findings と AC 判定 JSONL だけを渡す。
...
- Critical 候補あり: BB / WB の JSONL だけで Fresh Red Team を起動し再判定する。
...
AC 判定の ID 集合を期待する `AC-1..N` と比較する。不足・重複・未知 ID があれば1回だけ再取得し、なお不足する ID は `言及なし` で補完して欠落を報告する。
...
1. BB / WB / Red Team JSONL を合成し、coverage、severity、4分類、重複、Unknown と理由を分析ファイル末尾へ記録する。
