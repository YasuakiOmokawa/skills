...
**正規化ルール (全カテゴリ統一)**:
- 元の `- [ ] permission: ...` 行は **全カテゴリで観点ラベルを内部メタデータに展開** する:
  - 正常系 / 異常系: `- AC-N (カテゴリ, 観点: permission): 本文` (例: `- AC-1 (正常系, 観点: permission): 管理者が PATCH ... → 200 OK`)
  - エッジケース: `- AC-N (エッジケース, 観点: permission, 境界値: 本人): 本文` (例: `- AC-5 (エッジケース, 観点: permission, 境界値: 本人): 管理者が自分の role を PATCH → 422`)
- チェックボックス `[ ]` は除去、AC-ID は連番、観点ラベル `permission:` プレフィックスは保持しない (展開済みのため重複)
- 「正常系/異常系では観点ラベルを残す、エッジケースだけ展開」のような**非対称扱いは禁止** (subagent 側のパース実装で分岐が増えるため、全カテゴリで同形式)

```
${ENUMERATED_AC} の生成例 (全カテゴリで観点ラベル展開済み):
