...
**設計判断: Tier-based gating (案 2) は不採用** — 「軽量変更で main agent inline の AC quick check に置き換える」案を検討したが、core hypothesis (情報源分離 = Critical 検出 2.4 倍) を裏切るため不採用とした。修正版 (Tier 1 をテスト/docs/locale 限定) では軽量 path の本来の旨味が消えるため見送り。詳細は本 PR (#36) の議論を参照。
...
**Wiki Researcher と BB Analyst の wiki 探索の分担**:
- 並列起動を維持する都合上、BB の wiki 読みは**カレントリポのみ**に制限する (関連リポは Wiki Researcher 専属)
- BB が `read_wiki_*` を呼べる対象は dispatch 時に渡される `${REPO_NAME}` のみ。関連リポ (`${RELATED_REPOS}`) の wiki は BB は読まない (Wiki Researcher 結果を main agent が後段で統合する)
- これにより BB と Wiki Researcher の重複読みが最小化される (BB はカレントリポ wiki + 仕様で判定、Wiki Researcher は関連リポ wiki 全体)
