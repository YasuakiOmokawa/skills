...
抽出失敗時のリカバリと dispatch prompt template は [references/dispatch-prompts.md](references/dispatch-prompts.md)。**二重用途**: BB / WB の元 Markdown 全文は main agent 側で別途保持 (Step 3-3 で `<details>` 埋め込み用)。**2-2 結果受信**: `${RED_TEAM_RESULT}`。**Task ツールが利用不可な場合**: Red Team subagent を dispatch できないため [references/synthesis-and-errors.md](references/synthesis-and-errors.md) の「Red Team subagent 失敗」節のフォールバックに従う。
