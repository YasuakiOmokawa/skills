...
5. **判定不能は Unknown で棄権** — finding、AC 判定、コード証拠、または変更種別が具体的に関与を示す area だけを対象にする。対象 area の証拠が弱い・相反するなら原則 4、必要証拠が無ければ Unknown。関与シグナルのない一般領域を列挙せず、レビュー範囲外と1行で記録する。

「最低 N 件出せ」のような件数縛りは**なし**。該当があれば指摘、0 件なら根拠 1 文で明示すること。

## Critical 閾値 (BB / WB と統一)

[analyst-contract.md](analyst-contract.md) の `Critical 閾値` をそのまま適用する。

## クロスリファレンス 4 分類 (主機能)

BB Analyst と WB Analyst の各指摘について、両者の言及状況で 4 分類する。

この4分類で `—` を「言及なし」と扱えるのは `MISSING_ROLES=なし` のときだけ。片側ロールが欠落している場合、その不在から `実装漏れ` / `仕様漏れ` / `お見合い` を作らない。取得済み側の finding は元 source と severity を保って統合 Severity blockへ残すが、欠落側との X 行は生成しない。欠落ロールは `AC マージ検証` に1回だけ記録し、追加の再取得を指示しない。
...
2. **同 area 内で BB↔WB pair 検出**: 同じ area に BB と WB の両方が指摘を出している場合、それらは「真の合意」または「補強し合う合意」の候補。Issue 本文の意味的一致で最終判定。**area タグは絞り込みのヒントに留める** — BB / WB は同一主題に別 area を付けることがある (例: 税額算出を BB=business / WB=data)。同 area の pair が空振りしても、Issue 本文の意味的一致による照合を全 findings で必ず行う
3. **片側のみの指摘 (`MISSING_ROLES=なし` のみ)**: 同 area に BB のみ → 「実装漏れ」(コードに反映確認できず)、WB のみ → 「仕様漏れ」(コードの実装事実が仕様化されていない)。BB が**仕様自体の欠落**を指摘したケース (例: enum 未定義) も機械分類上は「実装漏れ」とし、content にその旨を注記する
...
取得済みの BB / WB のいずれにも言及がない領域を、以下のチェック観点から能動的に検出する。ロール欠落時は欠落側の不在を非言及の根拠にせず、取得済み finding または Read/Grep の正の関与シグナルがある場合だけ起票する:

**裏取り手段が全滅した area の扱い**: 関与シグナルがある area だけを Unknown にする。証拠ゼロの M 行は起票しない。関与しない一般領域は Unknown 件数にも加えず、レビュー範囲外とまとめる。
...
- 取得済みロールの AC 判定行数が dispatch 時の AC 数と一致するか確認 (不一致は subagent 不全のシグナル)。`MISSING_ROLES` のロールは retry 枯渇済みの不全として記録するが、再取得を指示しない。**この確認結果の出力先はレポートの「AC マージ検証」節のみで、不整合があるときだけ書く** (不整合 0 なら節ごと省略)
- 同じ AC-ID で BB「充足」/ WB「不十分」のように判定が割れる場合、両者の根拠を見て「コード上未実装だが仕様上は意図あり」(= 実装漏れ Critical 候補) か「コード上は対応しているが仕様文言が曖昧」(= 仕様明確化 Important) かを判別。**判別結果の出力先は 4 分類クロスリファレンス JSONL の該当行 (class / content) であり、「AC マージ検証」節には書かない** (同節は行数不一致 / 片側欠落の subagent 不全シグナル専用。判定割れの判別を書く場所として流用すると不整合 0 でも節が出現してしまう)
...
入力は取得済み BB / WB Analyst の **JSONLines** (findings + AC 判定) と `MISSING_ROLES` metadata。本チェックリストは:
...
**M 行の必須フィールド**: `id` / `area` / `perspective` / `content` / `severity` の 5 つすべて。特に `severity` を欠落させない (欠落すると main agent の統合 Severity 決定と output-format の「お見合い」行合成が壊れる。証拠不足で severity を確定できない項目は M 行にせず「判定不能 (Unknown)」へ)。
...
- レビュー範囲外: [関与シグナルのない一般領域を1行で列挙。なければ省略]

#### AC マージ検証 (Markdown、不整合があるときのみ)
- 取得済みロールの AC 判定行数が dispatch 時の AC 数と不一致、または片側がまるごと欠落している場合、ここに「subagent 不全シグナル」として明記する。欠落ロールは main agent の共有1回 retryが枯渇した terminal metadata なので、追加の再取得を指示しない (不整合 0 ならこのセクションごと省略)
