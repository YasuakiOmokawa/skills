...
4. **ロールバック不能** — revert できない変更・削除不能な外部影響。**コード差分の revert 可否と、既に外部へ渡った成果物 (送付済み請求書・送信済みメール等) の取り消し可否は別々に評価する** — 前者が可能でも後者が取り消せないなら該当 (法定書類・対外送信物の金額/内容誤りが典型)
...
**既存欠陥のスコープ**: 変更前から存在する欠陥でも、当該変更が新たに作るユーザー可視の主張・出力面でその害が顕在化するなら Critical 判定の対象に含める (例: 既存の集計バグを新設フッターが初めてユーザーへ露出する)。変更と無関係に存在し続けるだけの既存欠陥は範囲外 (Important 以下で記録)。
...
- BB と WB の AC 判定行数が dispatch 時の AC 数と一致するか確認 (不一致は subagent 不全のシグナル)。**この確認結果の出力先はレポートの「AC マージ検証」節のみで、不整合があるときだけ書く** (不整合 0 なら節ごと省略)
