...
1. **データロス / 破壊的変更** — 既存データの消失・不可逆な書き換え
2. **能動的に成立するセキュリティ侵害** — それ単独で不正アクセス / データ取得 / 権限昇格が **成立する** 欠陥 (IDOR / SQL・command injection / 認証バイパス / mass-assignment 権限昇格 / stored XSS 等)
...
BB / WB が返す「AC 判定」テーブルを main agent が AC-ID で join して総合判定を決定する。Red Team はマージ自体に介入しないが、不整合があれば指摘する。
