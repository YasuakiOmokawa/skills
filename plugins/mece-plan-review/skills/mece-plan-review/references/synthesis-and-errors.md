...
1. `dispatch-prompts.md` で期待 ID 順・N行・一意に正規化した**取得済みロール**の「AC 判定」テーブルを AC-ID で join。未知・重複行をここで選び直さない。retry 後に空となったロールは synthetic JSONL を作らず、各 AC のロール列だけ `未取得` とする
2. 各 AC-ID について以下の **AC カバレッジ機械合成ルール (本ファイルが SSOT)** で総合判定 (総合判定は 3 値文字列のみ: `充足` / `不十分` / `言及なし`、絵文字エイリアス禁止。`未取得` は欠落ロールの表示状態であり Analyst judgment ではない):
   - どちらか「不十分」 → **不十分**
   - 両方「言及なし」 → **不十分** (お見合い検出対象)
   - 少なくとも一方「充足」+ 他方「充足」or「言及なし」 → **充足**
   - 片方「未取得」→ 取得済み側が「充足」なら **充足**、`不十分` / `言及なし` なら **不十分**。欠落を仕様漏れ・実装漏れ・お見合いの証拠にしない
   - 両方「未取得」→ `dispatch-prompts.md` の terminal route。合成・書き込みを行わない
   - 両ロール「充足」要件にしない理由: BB は仕様観点・WB は実装観点。片方が言及なしでも他方が充足なら「不十分」とは言えない。Red Team が独自に懸念を検出した場合のみ「不十分」に格下げする
3. 元 AC 文の「カテゴリ」(正常系/異常系/エッジ/不変条件/非影響) を分析ファイルの AC セクションから補完
...
## Step 3-3: MECE 分析結果セクション upsert

`references/output-format.md` のフォーマットに従い、既存の `## MECE分析結果` 全体を置換して1節だけ保持する。無ければ**分析ファイル**末尾に追加する。

「各ロール出力 (JSONL)」セクションには取得できた `${BB_JSONL}` / `${WB_JSONL}` のみを `<details>` で格納する。空ロールの synthetic JSONL は作らず、分析サマリーの `未取得ロール` 行で理由を記録する。元 Markdown 全文 (Self-report 等) は転記しない (分析ファイル肥大とコンテキスト保持コストの主因だったため廃止済み。詳細は output-format.md)。

## Step 3-4: プランファイルに 1 行サマリー追記

プランファイルの `## 品質検証` セクションに以下を upsert する (セクション無ければ作成):
...
executor の戻り値がエラーまたはタイムアウト:
  → 該当ロールを [未取得] として記録
  → Red Team に欠落ロール名を伝え、残りの JSONL のみで Step 2 を継続。欠落を `言及なし` や漏れ分類へ読み替えない
```
