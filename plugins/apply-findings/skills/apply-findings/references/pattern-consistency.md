...
統一先の優先順 (auto-fix の統一先決定) と「明示的な指定」の定義は **SKILL.md「Findings の情報源」の パターン逸脱 節が SSOT**。本ファイルは統一ルール本体を持たず、混在の観点・言語別の典型例のみを担う。
...
4. **統一方針を決定**
   - 統一先の優先順は SKILL.md「Findings の情報源」の パターン逸脱 節が SSOT (規約 > 同一ファイル多数派 > 同一ディレクトリ多数派 > 判断系)
