...
以下は、AskUserQuestion が利用可能ツールに無い実行文脈 (= subagent として Task 経由で起動された場合) にのみ適用する読み替え。単独起動 (ユーザーがメイン会話で直接本 skill を起動した場合) の動作は変えない。**subagent として起動された場合は、Start here に入る前に [references/delegated-execution.md](references/delegated-execution.md) を必ず読む** — 対象特定・仮定ランキング確定・既存 ledger 再開・他 skill 呼び出し・Code-A standalone 化・完了報告の 6 点の読み替えを収録している。
