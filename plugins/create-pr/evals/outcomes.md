...
- fixture: ローカルの使い捨てリポジトリと固定応答を使う。リポジトリに `.github/PULL_REQUEST_TEMPLATE.md` があり、`~/.claude/skills-config/release-labels.md` 相当の指定設定は 3 つのラベル群と milestone 名を定義する。固定応答のラベル一覧は 3 群すべてのラベルを含むが、定義された milestone はリポジトリに存在しない。依頼は「下書き PR を作成して」である。実在する外部状態は変更してはならない。
- assertions:
  - [critical] PR 本文をテンプレートの見出し構成に沿って要点のみで書く。
