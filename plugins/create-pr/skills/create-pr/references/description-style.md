...
description 本文を書き始める **前に**、PR の本質を **bullet リスト** (点数は SKILL.md tier 表が SSOT) として scratch 出力する。各 bullet は「何を spec で固定するか / 何が動くようになるか / どのバグが解消するか」のような **読者目線の事実** を 1 行で。
...
- なければ standard の点数は「やったこと」**1 文に畳み込む**（各点の語彙を保持。[A] の本質リスト一致は「各点が文中に復元可能か」で判定）。1 文に収まらない deep の点数のみ「以下 N 点を実現する」+ 番号リストに格上げし、この格上げ番号リストは [C] の bullet 禁止・書式表「1 文 50-80 字」の**明示的例外**とする
...
- [ ] Pre-work で書いた「本質リスト」(点数は tier 表) と「このPRでやること」(または「やったこと」) の内容が**一致**しているか
