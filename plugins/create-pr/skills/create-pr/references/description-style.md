...
description 本文を書き始める **前に**、PR の本質を **bullet リスト** (点数は SKILL.md tier 表が SSOT: standard 2-3 点 / deep 5-7 点) として scratch 出力する。各 bullet は「何を spec で固定するか / 何が動くようになるか / どのバグが解消するか」のような **読者目線の事実** を 1 行で。
...
- なければ standard の 2-3 点は「やったこと」**1 文に畳み込む**（各点の語彙を保持。[A] の本質リスト一致は「各点が文中に復元可能か」で判定）。1 文に収まらない deep の 5-7 点のみ「以下 N 点を実現する」+ 番号リストに格上げし、この格上げ番号リストは [C] の bullet 禁止・書式表「1 文 50-80 字」の**明示的例外**とする
...
リストが 5 個以上に膨らむ場合は PR スコープが広すぎる兆候 — **ただしこれは standard tier の既定**。**deep tier (migration / breaking / multi-domain 等) では 5-7 点が正常な分解結果**で scope 過大ではない (SKILL.md tier 表が点数の SSOT、deep が優先)。standard で 5+ なら PR 分割を再検討するか「やらなかったこと」候補に振り分ける。
...
- [ ] Pre-work で書いた「本質リスト」(tier 依存: standard 2-3 点 / deep 5-7 点) と「このPRでやること」(または「やったこと」) の内容が**一致**しているか
