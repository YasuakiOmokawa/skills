...
6. **PR説明の作成**
   ...
   **「動作確認結果」のセクション構造（必須）**:
   ...
   - **trivial change 例外**: 変更が typo / コメント / 文言修正のみで実行検証や追加レビュー観点が自明に存在しない場合は、`目視確認のみ` / `自明な観点なし` のような固定 fallback 文言で 1 行を埋めるか、見出しのみ残し空欄（直下に空行 1 行）にする。**自明な「テスト OK」「特になし」を捻り出して埋めるくらいなら fallback / 空欄を選ぶ**
