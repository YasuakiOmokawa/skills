...
6. **PR説明の作成**
   ...
   **◆◆ 0 ステップ目: 変更の本質を 2-3 点リストとして書き出す（mandatory pre-work、文体原則より先に実行）◆◆**

   description 本文を書き始める **前に**、PR の本質を **2-3 点の bullet リスト** として scratch 出力する。各 bullet は「何を spec で固定するか / 何が動くようになるか / どのバグが解消するか」のような **読者目線の事実** を 1 行で表現する。

   これを最初にやる理由:
   - plan / design doc から派生する PR では、設計時に使った独自語彙（層番号 / AC 番号 / フェーズ用語 / ギリシャ文字ラベル）が無意識に description に混入する。「本質リスト」を作る習慣が plan 由来語彙の混入を物理的に阻止する
   - 「やったこと」「このPRでやること」「なぜやるのか」を独立に書こうとすると同じ概念を 3 回違う角度で書く重複が発生しやすい。本質リストを真の SSOT にすることで、各セクションは「リストを利用者目線 / 事実 / 動機にどう翻訳するか」の単純作業になる

   出力先:
   - テンプレートに「このPRでやること」のような **本質列挙系セクション** があれば、そこに本質リストをそのまま貼る（1-3 個の番号付きリスト形式）
   - なければ「やったこと」を「以下 N 点を実現する」+ 1 行サマリーの構成に格上げする

   OK 例（実 PR ベース、spec 追加 PR の場合）:
   ```
   ### このPRでやること

   以下 2 点を request 層で固定する spec を追加する。
   1. **enqueue 条件**: PR4-b で変えた `SyncFreeeLicenseJob` の enqueue 条件（OIDC 連携なしでも enqueue する / `team.freee_company` が nil なら enqueue しない）が caller-site で正しく動くこと
   2. **team 隔離 (PSIRT)**: URL にログインチームと別チームの id を入れてアクセスしても、`Ebis::Client.new` にログインチームに紐づかない freee_company_id が渡されないこと
   ```

   NG パターン（plan 由来の internal 語彙を持ち込んでしまった例）:
   ```
   ### このPRでやること

   PSIRT 4 層担保 (γ. CustomCop / β. DB schema / α. request spec / 構造) のうち α 層を新規追加し、PR4-b でマージ済の caller 挙動 (...) を request 経路から固定する。
   ```
   - 「4 層担保」「α 層」「γ/β/α」「caller 挙動」のような plan の internal 番号体系・抽象動詞で本質を覆い隠している
   - レビュアーは「α 層って何?」「caller 挙動の何が固定されるの?」と plan を読まないと解釈不能
   - 改善: 上記 OK 例のように **「何が caller-site で固定されるか」を読者目線の具体 fact 2 点に分解** して列挙する

   本質リストが 5 個以上に膨らむ場合は、PR スコープが広すぎる兆候。リスト整理段階で「やらなかったこと」候補に振り分けるか、PR 分割を再検討する。
   ...
   **◆ 簡潔セクション（斜め読みできる構造で書く）**
   ...
   各セクションは「役割 / 書式 / NG」を以下に集約。**コードから読める事実（変更ファイル名・関数名・パラメータ追加・import 追加・helper 名・依存削除対象モジュール）は書かない**は全簡潔セクション共通の鉄則（鉄則 1 参照）。

   | 見出し | 役割（これだけ書く） | 書式 | NG / 退避先 |
   |---|---|---|---|
   | 関連 Issue, Design Doc | チケット・先行 PR・DD のリンク | 単一なら 1 行（`related #1234`、推測不能なら `related -`）、複数なら bullet | 内容要約しない |
   | やったこと | PR 全体の意図（タイトルの文章化版） | **1 文 50-80 字、句点で締める** | 動機・採用パターン・代替案・比較対象（`〜と同じ pattern`）・括弧補足（`(cookie 検証スキップ)`）は書かない。退避先: 設計理由 → 「設計判断」、スコープ外 → 「やらなかったこと」、無ければ捨てる |
   | なぜやるのか | 利用者目線の動機（誰がなぜ困っていたか / 何を可能にしたいか） | 1 文または 1 段落 2 行以内、bullet 化しない | 「やったこと」の言い換え禁止。書く前に 1 往復読み比べる |
   | レビューしてほしい観点 | 不安箇所・判断を仰ぎたい点 | 1 行（例: `eager_load の選択が適切か`） | 「テスト OK」など自明事項 |
   | 動作確認結果 | 確認手段 → 結果の事実 | 1 行（例: `curl で cookie 有/無 5 ケース全て期待通り` / `Chrome MCP で landing → ログイン → /setting/profile 着地まで通過`） | ケース列挙 bullet。「spec 追加した」など diff から読める事実。N 件あるなら「全 N 件期待通り」で潰す |

   **trivial change（typo / コメント / 文言修正）の「やったこと」例文**（変更意図 + 修正前後を 1 行に）:
   - typo: `README のセットアップ手順の表記 typo を修正した。`
   - コメント: `OrderService の責務記述コメントを実装と一致するよう更新した。`
   - 文言: `エラー画面のメッセージを「サーバーエラー」から「処理に失敗しました。時間をおいて再度お試しください」に置換した。`

   **動作確認結果の判断マトリクス**（change 規模 × context 検証情報。**「テスト OK」「特になし」を捻り出すくらいなら fallback / 空欄を選ぶ**）:
   ...
   **複合見出しの記載例（`## 関連 Issue, Design Doc`）** — 複数項目は bullet、単一なら 1 行:
   ```
   - XPROJ-444
...
9. **PR本文の最終チェック（投稿前必須）**
   ...
   **[A] 斜め読みテスト（最重要・最初に実施）**
   ...
   - [ ] ステップ 6 冒頭で書き出した「本質 2-3 点リスト」と「このPRでやること」(または「やったこと」) の内容が**一致**しているか。リストに無い概念が膨らんでいないか、リストの bullet が削られていないか
   - [ ] plan 由来の internal 番号体系（例: `α 層` / `AC-9` / `Critical-A` / `§設計詳細` / ギリシャ文字ラベル + 「層/相」）が description に残っていないか。残っていたら読者目線の表現に書き換える（**[B] と独立に必ずチェック**: コード由来でなく plan 由来の語彙は B では検出されない）
