...
```bash
mkdir -p docs
cat > docs/billing-design.md <<'EOF'
# 課金サブシステム設計

## 認証
課金 API は既存の社内 IdP が発行する JWT を Authorization ヘッダで受け取り、`aud` が `billing` の場合だけ受理する。トークン検証は共通ミドルウェア `app/common/auth.py` が担い、課金側では再検証しない。
...
/create-design-doc にすすんで `docs/billing-design.md` に成果物を生成。根拠は `billing/retry/poc.md`。実装時はこの成果物のみ見て実装できる状態にして。
