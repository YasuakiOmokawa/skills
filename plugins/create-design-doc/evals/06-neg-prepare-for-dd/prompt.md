...
`billing/retry/poc.md` にまだ Prototype 節が無い。プロトタイプの結果は次のとおりなので、`## Prototype` 節として追記して、create-design-doc ができる状態にしてほしい。
- 適合性: `app/billing/charge.py` の `ChargeClient.charge` に冪等キーを渡す既存契約に合致
- 検証: 既存テスト pass、再試行 3 回で二重請求 0 件を再確認
