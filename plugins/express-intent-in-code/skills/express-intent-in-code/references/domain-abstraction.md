...
- **Step G — 行き詰まりかつ human reachable なら escalate**: greenfield 等でどの artifact からも語が出ず、ユーザーに聞ける状況なら、**証拠付き候補表を提案語として提示して確認を仰ぐ** (Event Storming/Example Mapping の red-card の代替)。未確認の語を単独判断で型名に固めない。human reachable でない場合 (= AskUserQuestion が利用可能ツールに無い実行文脈) は確認を仰がず、下記「据え置きの記録」へ進む。
