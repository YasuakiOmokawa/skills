...
- 対象が引数で明示されていなければ、まず `/review-code-quality` の申し送りファイル `$(git rev-parse --git-common-dir)/quality-review-handoff-$(git branch --show-current | tr '/' '-').md` (無ければ旧パス同ディレクトリの `quality-review-handoff.md`) を探し、naming / 凝集 finding があればそれを対象として読み込む。**読み込むだけでクリアはしない** (クリアは `/polish-before-commit` の責務)。申し送りファイルが無い、または該当 finding が無い場合にのみユーザーに確認する (branch 名や推測で決め打ちしない)。
...
- **新規 public 型フィールド・公開シンボルの命名を含む場合は、単一命名でも非自明側に倒し intent-reader を既定とする** (理由: 実測 2026-07-17 — 自己命名は cold self-read では常に明瞭に見える。比喩語 `anchor` が cold self-read と品質パス連鎖を素通りし、利用者の初見質問 2 回で 2 段階の改名に至った)。
