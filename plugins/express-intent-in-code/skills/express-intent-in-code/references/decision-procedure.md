...
- 引数で対象が 1 点に確定していなければユーザーに確認 (branch 名や推測で決め打ちしない)。
...
- 段4 は上限であって必須ゴールではない。共有ドメイン語が無ければ段3 で据え置き、造語しない。
...
- 非自明な対象は [agents/intent-reader.md](../agents/intent-reader.md) を Task で起動 (bias-free な readability test)。自明な単一改名は cold self-read で代替。
