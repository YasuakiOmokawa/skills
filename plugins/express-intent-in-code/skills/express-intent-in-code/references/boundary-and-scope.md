...
**スコープ内**:
- 対象が確定した working code 1 点 (メソッド/値/型) を受け、名前/型/構造/テストを why 表明形へ**深く変換**する。出力は before/after 変換・改名 3 案・昇格削除コメント一覧・残す真 why。
...
- 段4 ドメイン抽象への到達を必須化すること → 段4 は上限。段3 で意図が十分なら据え置き、共有語が無いのに造語しない。
...
| 広さ×深さ | 広く浅く (凝集/結合/可読性 4 観点) | 狭く深く一点突破 |
| 出力 | 指摘リスト + 重大度 | before/after 変換 + 改名 3 案 + 昇格/残置コメント |
| 自動適用 | readability 軸の機械的に安全なものだけ | 変換が本体 (lint/test で都度検証) |
| トリガ | self-review 完了時・PR 前・domain attribute 更新時 | 対象が判明し「目的を名前で表明したい」時 |

## 非重複の保ち方 (4 項)

1. **診断基準を再掲しない**: review-code-quality が既に持つ ambiguous name / boolean 否定形 / 凝集度・結合度スコアリング / lint で拾える naming は本スキルに転記せず、診断は委譲する。
...
4. **Surgical Changes の継承**: 動くコードの命名を要求なく触らない。対象メソッドが今回の変更対象である時に限り適用する。
