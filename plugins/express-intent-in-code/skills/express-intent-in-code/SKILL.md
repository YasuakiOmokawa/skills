...
**バッチ / パイプライン起動 (単一対象が未指定のとき)**: `/simplify` 等の品質パイプライン後段で、単一対象も `/review-code-quality` からの naming/凝集 handoff も無いまま起動された場合 — diff 全体を改名候補スキャンせず、handoff があればそれを変換対象に取り、handoff も明示対象も無ければ「handoff 無しのため変換対象なし」と即 no-op 宣言する (無人パイプラインを確認待ちで止めない)。同一実装セッションで経路2 を適用済みでも、この本呼び出しは経路1 として独立に実行する (経路2 実施済みを理由に no-op としない)。**例外**: handoff の有無によらず、機械検査で拾える 3 兆候 — (a) 多義衝突、(b) 新規識別子の段0 ノイズ語 (`doc`/`data`/`info`/`target`/`tmp` 等)、(c) 新規/移植ファイルのコメント過剰 (非空行に占めるコメント行が目安 25% 超) — だけは Step 6 の対象に加える。3 兆候の判定手順・副次候補の据え置きログ・mv を伴う変換前の handoff/ledger 再読は [references/batch-invocation.md](references/batch-invocation.md)。
