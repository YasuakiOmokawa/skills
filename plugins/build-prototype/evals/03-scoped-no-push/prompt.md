...
- 未解決事項: なし
- `app/common/text.py` の `normalize` は lower なので casefold に変えて共用したい
- 参照番号: POC-SP-7731
EOF
```

`plans/search-prefix/poc.md` を読んで、承認済みの PoC を `app/search` の既存構成に沿うプロトタイプへ書き直してください。編集を許可するのは `app/search` 配下とそのテスト、`plans/search-prefix/poc.md` だけです。それ以外のファイル、公開先、リモートなどの外部状態は変更しないでください。
