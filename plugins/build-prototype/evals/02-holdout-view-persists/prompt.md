...
- 未解決事項: なし
- 動作確認は次の形で行った (handlers から索引を直接引く)。移植時もこの形で動く
  from app.search.store import RecordStore
  from poc.search_prefix.prefix_index import PrefixIndex
  _index = PrefixIndex(RecordStore().all())
  def handle_search(params):
      items = _index.search(params.get("q", ""))
      return {"items": items, "count": len(items)}
- 参照番号: POC-SP-7731
EOF
```
