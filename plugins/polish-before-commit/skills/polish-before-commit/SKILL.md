...
```
5. **一覧を提示したらここで本 skill は終了する。** agent 側から commit / `git add` / `/create-pr` を自発実行・提案しない (フロー開始時点で既にコミット / PR を指示済みなら、その指示に従ってよい)。判断項目の有無で終了時の文言を分ける (自発 commit はしない原則はどちらも同じ):
