...
description: Auto-fixes convention and pattern-consistency issues, runs lint, and aggregates remaining judgment calls before stopping for the user. Requires the Claude `feature-dev` plugin for the final review step and halts with install guidance when it is missing. Use when finalizing a branch, just before `git commit` or `/create-pr`, or whenever the user says "仕上げて" / "polish" / "コミット前チェック".
---
...
Task ツールが使えない (nested 実行) 場合のみ、main thread で同等のレビュー (変更 diff のバグ・規約違反確認) を直接行い、`[最終レビュー: ... (fallback)]` と明示する (silent skip 禁止)。`feature-dev` 未導入は Step 0 で中止済みなので、ここには未導入状態で到達しない。
...
3. 採用した申し送り項目 + 本 skill の Manual Review Items (前掲・tier 表直後) を統合し、同一箇所の重複は 1 件にまとめる。
...
```
5. **一覧を提示したらここで本 skill は終了する。** 判断項目の有無にかかわらず (0 件でも同じ)、agent 側から commit / `git add` / `/create-pr` を自発実行・提案しない。「polish 完了。コミットへ進めますか?」と 1 文返してユーザーの明示指示を待つ (フロー開始時点で既にコミット / PR を指示済みなら、その指示に従ってよい)。
