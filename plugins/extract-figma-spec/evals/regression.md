...
Step 5.1 の「AC（またはプランの反映チェックリスト）」が AC 化後も併存可能かは明文化されておらず、executor は毎回両方作成して自己解決していた (長期未解決点だった)。

追記 (2026-07-11, 週次利用履歴改善 PR): 上記の併存可否を Step 5.2 に明文化した際、初版の文言「AC 化した場合は重複転記不要」を fresh executor が「AC 化を決めた時点で転記省略可」と読み、AC 未生成のまま差分がどこにも転記されない critical FAIL が Step 5 分岐 hold-out の再実行で発生した。「実際に実行して AC が生成された後は転記不要 (未生成のうちは省略しない)」へ修正後、同 hold-out と PoC 経路 hold-out の 2 ラウンド連続で全 [critical] ○ を確認して収束。
...
収束記録: 2026-07-17 (v0.6.0 progressive disclosure 分割)。Step 3 の重複 atom 統合基準・保留手順を references/checklist-building-details.md へ verbatim 退避 (挙動変更なし)。全 4 シナリオを fresh executor で再実行し全 [critical] ○。`## 正本抽出結果` / `| atom ID | 期待値 | 状態 |` の skill 間契約書式が維持されていることを確認。

収束記録: 2026-07-18 (orchestrated regression 再実行、skill 無改変の劣化検出ラウンド)。実 Figma MCP に接続できない eval 環境のため、シナリオ設計に mock 記載の無い MCP 応答は妥当な台本を executor prompt に埋め込んで再現した (A/B: `get_metadata`(nodeId なし) が接続不達エラーを返す、C: 接続成功しページ一覧 `Priority Tab v0 / v1 / Components / Cover` を返し v0/v1 の版ラベル一致で一意化不能になる Gotcha を踏ませる形)。全 4 シナリオ (委譲 A/B、ドリルダウン hold-out C、Step 5 分岐 hold-out) を fresh executor で再実行し全 [critical] ○ / accuracy 100% / tool_uses 2-4 / duration 80-102 秒。A/B/C の出力先プランファイルは grep で `## 正本抽出結果` / `FIG-` 0 件を確認 (架空データ非書き込み)、Step 5 は 8 行テーブル + 差分 3 件 (FIG-03/06/07) の反映チェックリスト転記をファイルで確認。executor が挙げた軽微な文書精度の指摘 (初回接続確認 Step 1-1 の retry 要否、run dir 内既存ファイルの出力先流用可否、v0/v1 版指定) はいずれも既存 Gotchas / 委譲実行の入力解決 ③ / 版ラベル Gotcha が既に回答済みで、executor は本文規則を引用して自己解決しており新規の critical 欠陥ではない (2026-07-07 にエスカレーション済みの documentation-precision jitter と同系)。前回 2026-07-17 の収束が直前ラウンドのクリアとして先行するため本日 1 ラウンドで確定。SKILL.md / references への修正なし。

記録: 2026-07-25 (opus5/fable5 静的最適化パス)。Iter 0 (description/body 整合) gap なし。ルーブリック走査の結果、無変更と判定: ⚠️ はハザードゲート (スクショ非正本・部分集合警告)、「明示 none/沈黙」の腑分けは中核決定規則、Gotchas 2 件は観測起点。検討して見送った項目 1 件: Pitfalls 節 6 項目中 5 項目がインライン規則の再掲 (recap) で drift リスクがある — ただし前回 tuning がこの構成を維持した経緯があるため、empirical 検証なしでの削除は行わず次回 tuning の検証候補として申し送り (削除する場合は #6 MCP 切断リトライのみ残す。Gotchas が参照しているため)。empirical 検証は session の subagent dispatch 上限到達のためスキップ (利用者承認済みの縮小完了)。
