...
description: Figma デザインをコードへ反映・適用する実装の前後、figma-dev-mode MCP で design-to-code を行うとき、または「Figma 通りに実装して」「デザインを反映して」「Figma の指定どおりに直して」と頼まれたときに使用。スクショ目視と人間が挙げた「指定 N 点」だけで反映すると、hex の微差・font weight(Bold)・アイコンや枠の有無・要素まるごとの欠落 といった Figma 指定を取りこぼす——この反映漏れ (デザイン崩れ・Figma 不一致) を防ぐ。差分は /define-acceptance-criteria で AC 化し /qa-ui で検証する。チェックリスト全行は atom ID 付きで `## 正本抽出結果` として分析ファイルに書き出し、/finalize-plan の正本カバレッジ・ゲートの入力になる。
argument-hint: [対象 Figma ノード / URL（省略可）]
---
...
1. `mcp__figma-dev-mode-mcp-server__get_metadata` を呼ぶ（接続確認を兼ねる）。
   - 失敗 → 以下を表示して**停止**: 「Figma Dev Mode MCP に接続できません。Figma デスクトップアプリが起動し、対象ファイルが開いているか確認してください。」
2. 対象ノードを確定する:
   - `$ARGUMENTS` に node-id / URL がある → そこから node-id を抽出（URL `...?node-id=1-2` → `1:2`）
   - プラン / セッションに対象ノードが既出 → それを採用
   - 不明 → `get_metadata`（nodeId なし）が返すトップレベルページ一覧から辿り、対象ページ → 対象ノードへドリルダウンする
3. 反映対象が複数ノードに跨る場合は node-id を**全て列挙**してから Step 2 へ。1 ノードずつ処理する。

### Step 2: ノードごとに構造化データを抽出（スクショは補助）

各対象ノードについて、次の 4 つを取得する:

1. `get_metadata`(nodeId) — ノード木（子要素・レイヤー種別・名前・位置・サイズ）。**子要素を列挙して「要素まるごとの欠落」を防ぐ**起点。
...
2. `/define-acceptance-criteria` で受け入れ条件化し、`/qa-ui` で実装後に 1 行ずつ pass/fail 検証できる形にする。
3. チェックリストの全行（一致を含む）を `<plan>.analysis.md` の `## 正本抽出結果` セクションに書き出す。分析ファイルが無ければプランファイル末尾に同セクションを追記する。列は atom ID・期待値・状態（`一致` / `差分` / `未実装`）の 3 列（カテゴリは期待値の文言に含めて書く。例: 「左ペイン背景色 #464343」「IconOnlyButton は枠なし」）。フォーマット:
