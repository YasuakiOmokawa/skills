...
- **AC セクションが空 / 全項目分類不能** → AskUserQuestion で「AC が分類できません。`/define-acceptance-criteria` を再実行するか、AC を手動で正常系/異常系/エッジケース/非影響/[MECE追加] にラベル付けしてください」と確認する (AskUserQuestion が使えない実行文脈での読み替えは SKILL.md「## 委譲実行」参照)
