# Agent orchestration (Step 2B)

全体構成: **Step 2A はインライン (agent 起動なし) → Step 2B で manual-qa + auto-qa を並列**。起動する agent は 2 つだけ。

Step 2A (ブランチ戦略) は main agent が SKILL.md の手順に沿って直接行い、結果を `${BRANCH_RESULT}` として保持する。Task 起動しないため本ファイルにレシピは無い。
...
Step 1.7 で `${ENUMERATED_QA_AC}` を先に生成してから Step 2B を起動する順序は維持する (両 planner がこの enumerate 結果を入力に使うため)。
...
1. 2 agent 定義 (`${CLAUDE_PLUGIN_ROOT}/skills/finalize-plan/agents/{manual-qa-planner,auto-qa-planner}.md` — SKILL.md「## 委譲実行」の `${CLAUDE_PLUGIN_ROOT}` 解決規則で導いた絶対パスを使う。Read は絶対パス必須のため相対パスのままでは読めない) を Read で順次読込。ブランチ戦略は通常モードでも main agent がインラインで行うため fallback でも読込対象に含めない
2. 本 agent 自身が各 agent の判定基準・出力フォーマットを適用し、QA サブセクション (手動 QA / 自動 QA) を内部生成 (中間出力なし)。ブランチ戦略は Step 2A の手順をそのまま適用する
