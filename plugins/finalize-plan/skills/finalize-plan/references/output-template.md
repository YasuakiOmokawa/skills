...
正本カバレッジ: <skip / 差分 0 件 (...) / 補完 N 件 (再実行で差分 0 件) / 未カバー N 件 (<atom IDs>)>
...
他の canonical 行は `正本カバレッジ: 差分 0 件 (<counts>)`、`正本カバレッジ: 補完 N 件 (再実行で差分 0 件)`、`正本カバレッジ: 未カバー N 件 (<atom IDs>)`。未カバー行は未完了を表す。

coverage 行は manual 末尾、auto 見出し直前に置く。fallback 備考は `## 実装準備` 直下に置く。
QA source marker は Step 5 後に ordered `(QA-ID, 手段, exact source)` から生成し、`## 実装準備` の末尾へ `<!-- QA source: <sha256> -->` と1行だけ upsert する。
