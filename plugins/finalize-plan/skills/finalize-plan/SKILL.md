...
7. **Step 3.5**: 正本カバレッジ・ゲート + PR 割当ゲート (Step 3 の Write 後、プランファイル自体を対象に実行)
...
${PLAN_CONTENT}
## Enumerated AC:
${ENUMERATED_QA_AC}      # qa planner・pr-splitter (PR ごとの QA-ID 割当に使う)
## MECE 分析結果:
...
**PR 割当ゲート (計測知見 7)**: 実測で、カバレッジマトリクスの auto QA-ID 23 件中 6 件がどの PR にも割り当てられないまま実装漏れとなり、QA ゲートで事後検出された。再発防止のため、正本カバレッジ・ゲートと同じタイミング (Step 3 の Write 後) でプランファイル自体を対象に機械検証する。**検証済み Bash** (fixture で実行検証済み):
...
if grep -q '^現ブランチ 1 コミット' "$PLAN_FILE"; then
  echo "PR割当ゲート: skip (single-branch / no-PR モード、PR分割なし)"
  exit 0
fi

# 1. auto QA-ID 全量: QA-ID カバレッジマトリクスの行から (Step 4 の auto 抽出と同一パターン)
awk -F'|' '/^\| *QA-[A-Z]+-[0-9]+ *\|/{id=$2; gsub(/^[ \t]+|[ \t]+$/, "", id); print id}' "$PLAN_FILE" | sort -u > /tmp/auto_qa_ids_all.txt

# 2. PR分割計画の「実装QA-ID」列に登場する QA-ID (PR行のみ対象。カバレッジマトリクス行 (「| QA-...」始まり) と区別するため「| PRn |」始まりの行に限定)
grep -E '^\| *PR[0-9]+ *\|' "$PLAN_FILE" | grep -oE 'QA-[A-Z]+-[0-9]+' | sort -u > /tmp/pr_assigned_qa_ids.txt

# 3. 真の集合差分
comm -23 /tmp/auto_qa_ids_all.txt /tmp/pr_assigned_qa_ids.txt > /tmp/qa_ids_unassigned.txt
if [ -s /tmp/qa_ids_unassigned.txt ]; then
  echo "PR割当ゲート: 未割当 $(wc -l < /tmp/qa_ids_unassigned.txt) 件"
  cat /tmp/qa_ids_unassigned.txt
  exit 1
else
  echo "PR割当ゲート: 全 $(wc -l < /tmp/auto_qa_ids_all.txt) 件割当済み"
  exit 0
fi
```

未割当 QA-ID が出た場合、その QA-ID を実装するタスクを含む PR の「実装QA-ID」列に Edit で追記する。該当タスクがどの PR にも見当たらない場合は PR 分割自体に抜けがある可能性が高いため、pr-splitter のタスク分類を見直す。追記後にゲートを再実行し未割当 0 件を確認する。ゲート結果 (`全 N 件割当済み` / `未割当 N 件` / `skip (single-branch モード)`) は `## 実装準備` に残す。**既存の `PR割当ゲート:` 行がある場合は最新の結果で置換する（重複追記しない）**（正本カバレッジ・ゲートと同じ理由: 行が積み重なると最新がどれか機械判定できなくなるため）。
...
- **QA-ID → PR 割当の完全性**: PR分割計画の「実装QA-ID」列に、カバレッジマトリクスの全 auto QA-ID が漏れなく載っている。Step 3.5 の PR 割当ゲート結果 (`全 N 件割当済み` / `未割当 N 件`) を成果物に残す (計測知見 7: 未割当のまま実装漏れが QA ゲートで事後検出された実測が根拠)
