...
1. プランファイルが無ければ作成し、冒頭を次のテンプレートに揃える (見出し行の
   直後に記録する。URL は doc ID と `tab=` までに正規化し `#heading=...` は落とす。
   `?tab=t.<tabID>` はスナップショット範囲の指定を兼ねる):

   ```
   # plan: <案件名>
   PRD gdocs: <URL>
   スナップショット範囲: タブ単体 (t.<tabID>)。子タブ包含: 未確認
   ## 進捗
   ```

   全文を凍結する場合は `スナップショット範囲: 全文` の 1 句でよい (全タブ包含済み
   のため「子タブ包含」句は書かない)。見出しの `<案件名>` はディレクトリ名と同一で
   よい (説明句の付与は任意)
2. エクスポートする。URL にタブ指定が無ければ
   ...
   - token は `rclone config dump` の出力 (drive セクション token の access_token)
     から取る。抽出と curl 実行は 1 コマンドに統合し、token 単体を出力・保存する
     中間ステップを作らない (`rclone.conf` の直接読取も dump の単独実行も
     credential 保護 hook に deny されうる — 2026-07-11 実測)。401 は
     `rclone about drive:` を一度実行して refresh 後に再取得、403
...
3. 画像定義行を
   ...
   で除去して `prd_gdoc_snapshot_<日付>.md` として保存する。生エクスポートは削除する
...
/qa-ui を automation で実行して   # Figma 条件とは独立に常に実行
...
コードコメントと md ファイルに /dry-ssot-text → /purge-private-vocab → ~/.claude/rules/japanese-writing.md と照合
...
DD に /dry-ssot-text → /purge-private-vocab → ~/.claude/rules/japanese-writing.md と照合   # レビュー依頼前に plan 造語を除染
...
/mece-plan-review
/finalize-plan
/dry-ssot-text → /purge-private-vocab → ~/.claude/rules/japanese-writing.md と照合
