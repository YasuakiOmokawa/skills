...
`Task` ツール (`subagent_type="general-purpose"`、本リポ標準の dispatch) で `ui-evaluator` を起動する。
...
1. QAエージェントの「修正の示唆」に基づき、**該当 AC を pass させる最小限の修正だけ**を行う（Opus 系は要求外のリファクタ・抽象化・余分なファイル生成に流れやすいため、失敗した AC のスコープを越えた変更は持ち込まない）
