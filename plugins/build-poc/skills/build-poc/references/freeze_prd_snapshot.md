...
1. プランファイルが無ければ案件ディレクトリ (既定 `~/.claude/prototyping-projects/<案件名>/`)
   ...
   ## 進捗
   (記帳をここに 1 行ずつ。呼び出し側が書くやりたいこと・実装対象リポジトリ・
   PRD の所在は、この記帳より後ろに別見出しを立てて置く)
   ```
...
2. エクスポートする。URL にタブ指定が無ければ
   ...
   - この「rclone token を curl に転用する」統合コマンド自体が、実行環境の
     ...
     (許可ルールは事前のユーザー同意なので対話不能でも使える。許可済みかは実行してみて
     判定する — deny が返れば未許可扱い。未許可が既知なら実行もしない。settings ファイルを
     読んで判定しない。導入 — chmod +x と
     ...
     (完了扱いにしない)
   - 経路は確定したが取得自体が終わっていない状態 (実行を後続セッションへ回した等) も
     未取得として記帳する。経路の決定と取得の完了は別イベントなので別行で書く
     (書式例: `- <日付> スナップショット未取得: <理由>。経路は確定済み (<経路>) だが
     取得は未実行 — 凍結は成立していない`)。この行がある限り凍結済みとして扱わない
...
4. 再取得 (2 回目以降 — /build-prototype 直前の取り直しや DD 時の最新版反映) は、
   全文スナップショットを並べず差分方式で更新する。凍結の判断材料として要るのは
   「何が動いたか」であり、全文の正史は gdocs named version (手順 5) が保全している
   (実案件で 250 行級の全文コピーが 3 本並び、どれが凍結基準か追う手間が生じた前例
   あり — 2026-07-31):
   - 新版を一時ファイルへエクスポートし (経路・範囲は `スナップショット範囲:` 行と
     同一 — 手順 2 の同一経路則。画像除去も手順 3 どおり先に済ませる)、既存
     スナップショットと `diff -u` で比較する
   - 差分なし → 既存ファイルを凍結基準に据え置いて一時ファイルを削除し、進捗欄に
     `- <日付> 再凍結: 差分なし (基準据え置き)` と記帳する
   - 差分あり → 一時ファイルを `prd_gdoc_snapshot_<日付>.md` (旧版と同日なら `_2`
     等の連番を付与) として保存して新しい凍結基準にし、旧スナップショットは削除する。
     プランファイルの `## PRD 差分履歴` 節 (無ければ新設) に `### <旧日付> → <新日付>`
     を立て、変更を 1 件 1 行 (変更箇所の見出し + 要旨) で記録し、AC・設計の再判断が
     要る変更にはその旨を行末に添える。進捗欄には
     `- <日付> 再凍結: 差分あり — PRD 差分履歴を参照` と記帳する
5. (人間: gdocs 側に named version を打ち Keep forever を付ける — 版は 30 日または
