...
検索実装の候補 A と B の技術選定 PoC をしてください。条件は `poc/search/README.md` にあります。出力先は `plans`、案件 slug は `search-index` にしてください。書き込んでよいのは `poc/search/` と `plans/` の下だけで、他の場所は読み取りのみです。この作業ディレクトリは git 管理されておらず、remote も PR 作成手段もありません。git リポジトリを新規作成しないでください。この作業ディレクトリの外は変更しないでください。
