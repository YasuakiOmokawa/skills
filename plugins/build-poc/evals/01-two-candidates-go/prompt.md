...
検索実装の候補 A と B の技術選定 PoC をしてください。条件は `poc/search/README.md` にあります。結果の出力先は `plans`、案件 slug は `search-index` にしてください。検証コードは `poc/search/` の下に置いてよいです。この作業ディレクトリの外は変更しないでください。
