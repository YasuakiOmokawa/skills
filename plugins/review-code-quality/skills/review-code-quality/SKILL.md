...
2. 処理方式を選ぶ — ファイル ≤ 2 は main thread 順次、> 2 かつ Task 使用可は 4 agent 並列 (同一メッセージ内に Task 4 つ)、> 2 かつ Task 使用不可は main thread fallback + 理由明示。分岐表と Task 可否判定は [references/execution.md](references/execution.md)
...
4 agent (並列モード) の**すべての結果を受信してから**開始する (部分結果での先行実行は禁止、root cause 集約の前提が崩れるため)。main thread 代替実行では 4 観点を順次完了してから進む。business-impact-analyzer の **skip 報告も統合レポートに残す**。
