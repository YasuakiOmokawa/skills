...
**コメント追記の上限**: finding 対応で新たに足すコメントは **Why 1-2 行まで**。責務の列挙・ivar/引数の契約列挙・経路の一貫性注記のような網羅的説明コメントの追加は auto-apply しない (名前とコードから自明な内容の列挙は水増しで、レビューで削られた実績がある)。1-2 行で書けない説明が要る finding は needs-judgment へ。analyzer の提案修正を main thread が別内容に縮小・差し替えて適用することもしない (差し替えが要る時点で修正方針が一意でない証拠 → needs-judgment)。
