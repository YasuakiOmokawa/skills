...
起動時に必ず以下を読み込む:
- `${CLAUDE_PLUGIN_ROOT}/skills/review-code-quality/references/business-impact.md`（`${CLAUDE_PLUGIN_ROOT}` が生文字列のままなら、この agent 定義ファイルと同じ `agents/` ディレクトリから見た `../references/business-impact.md` として読み替える）
