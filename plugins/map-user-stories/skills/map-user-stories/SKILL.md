...
⚠️ **「やらないこと」「完了条件」のプレフィックス記法を活用**:

`create-jira-issues` は Jira description を `着手条件 / やること / やらないこと / 完了条件` の 4 セクションで生成する。US は技術メモ列、タスクは備考・説明列にプレフィックスを付けて記述する:

- 技術メモ・備考列に `やらない: {スコープ外項目}` `対象外: {別タスク範疇}` で記述 → Jira「やらないこと」へ転記
- タスク説明列に `完了条件: {独立した完了条件}` `AC: ...` で記述 → Jira「完了条件」へ転記（未記入なら親US の AC を継承）
- 1 セルに複数行ある場合は ` ／ ` で区切る
...
- [ ] **やること**: ストーリー / タスク名+説明が空でない
- [ ] **やらないこと**: 技術メモ / 備考列に `やらない: ` / `対象外: ` 行が記載されている（明示推奨。空欄もフォールバック可だが現実には大抵 1 つはある）
- [ ] **完了条件**: 受入条件 / タスクの完了条件が記入済み（タスクは親US AC 継承で OK）
