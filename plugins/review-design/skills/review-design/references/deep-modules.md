...
1. **問題空間を提示** — sub-agent 起動前に、選んだ候補の制約・依存 (どの分類か)・制約を具体化する粗いコードスケッチ (提案ではなく制約の地ならし) をユーザー向けに書く。対話承認者がいる (= AskUserQuestion が利用可能ツールにある) 場合は読んでもらいつつ Step 2 へ。対話承認者がいない (= subagent 実行) 場合は提示のみ行い、確認を待たず即座に Step 2 へ進む。
