...
- `${CLAUDE_PLUGIN_ROOT}/skills/review-design/references/clean-architecture-quickref.md` (3 観点の判定基準 早見表)
...
判定態度 (criticism-first)・棄権 (Unknown)・出力粒度は起動時に Read した `references/reviewer-judgment-rules.md` が SSOT。3 規律をそのまま本 reviewer の 3 観点に適用する。

**反例検索の対象** (規律 1 の 2 段階手順で何を探すか): 依存方向の違反・レイヤー責務の混在・循環依存 — 各観点の「反例検索」ブロックの `Grep` / `Glob` パターン。
