...
- `${CLAUDE_PLUGIN_ROOT}/skills/review-design/references/hexagonal-architecture-quickref.md` (4 観点 + 採用判断表の早見表)
...
判定態度 (criticism-first)・棄権 (Unknown)・出力粒度は起動時に Read した `references/reviewer-judgment-rules.md` が SSOT。3 規律をそのまま本 reviewer の各観点に適用する。

**反例検索の対象** (規律 1 の 2 段階手順で何を探すか): 外部依存の直接参照・Port/Adapter の欠如・過剰適用 — 各観点の「反例検索」ブロックの `Grep` パターン。
