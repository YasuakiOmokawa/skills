...
**Row 3 と Row 4 の compound / Row 4 territory の core path 境界例** (領収書・ログイン UI・権限表示など周辺機能を Row 3 に落とす基準) は [references/task-tier-boundaries.md](references/task-tier-boundaries.md) を参照。
...
Task ツールで委譲起動された (= いま Task 経由で動いている) なら、単独起動の動作に委譲固有ルールが加わる。**進む前に [references/delegated-execution.md](references/delegated-execution.md) を必ず Read すること。** 判定基準は「AskUserQuestion が利用可能ツールに無いか」で機械的に行う。同ファイルが規定する項目: 入力解決順位 (`Plan File Info:` / 会話文脈は使わない) / 指定パスが存在しない・入力不足時の即時完結 (捏造せず待たず終了) / Step 3 の Task 不可時 fallback / Design It Twice の非対話進行 / `${CLAUDE_PLUGIN_ROOT}` 解決 / 完了報告。
