...
- Step 5.4「1-2 の hidden assumption を洗い出す」に対応する記載枠が Step 6 保存ファイル ([references/final-report-format.md](references/final-report-format.md)) のテンプレートに無い。executor は洗い出した前提を成果物のどこに書くか (Acceptable 残存リスク表に含める等) を自己裁量で決める必要がある。
