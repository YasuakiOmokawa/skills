...
**in-context 代替モード時の表記**: Step 3 (Parallel Review) または Step 5 (Devil's Advocate) のいずれか **1 つ以上** で fallback を使った場合、最終報告本文の**末尾に 1 行だけ**以下を追加する:

```
(in-context 代替モード: <代替したエージェント名をスラッシュ区切り>)
```

- 対象集合: Reviewer (4 種) **および** `devil's-advocate` を含む (Step 5 の Devil's Advocate も fallback 対象に含めること)
